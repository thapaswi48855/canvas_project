...
export class AppComponent implements AfterViewInit {
  ...
  redrawCanvas() {
    ...
    this.images.forEach(imageLayer => {
      context.drawImage(imageLayer.img, imageLayer.x, imageLayer.y, imageLayer.width, imageLayer.height);

      if (imageLayer.isSelected) {
        context.strokeStyle = 'blue';
        context.lineWidth = 2;
        context.strokeRect(imageLayer.x, imageLayer.y, imageLayer.width, imageLayer.height);
        ...
        handles.forEach(handle => {
          context.beginPath();
          context.arc(handle.x, handle.y, imageLayer.resizeHandleSize, 0, 2 * Math.PI);
          context.fill();
          context.stroke();
        });
      }
  ...
  onMouseDown(event: MouseEvent) {
    ...
    this.selectedImage = this.images.find(imageLayer =>
      x > imageLayer.x &&
      ...
      imageList.innerHTML = this.images
        .map(imgLayer =>
           `<li style="background:#f5f5f5;margin-bottom:5px;font-size: 20px;display: flex; align-items: center;"> 
            <img style="margin-right: 13px;" src="${imgLayer.dataUrl}" alt="${this.capitalizeWords(this.removeFileExtension(imgLayer.name))}" width="30" height="30" />
            ${this.capitalizeWords(this.removeFileExtension(imgLayer.name))}</li>`)
        .join('');
